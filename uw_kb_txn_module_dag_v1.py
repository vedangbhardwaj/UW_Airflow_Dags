...
def run():
    import time
    config_var = Variable.get("underwriting_dags", deserialize_json=True)["txn_module"]
    ...
    def model_prediction(dataset_name,**kwargs):
        ...
        print(f"**************{Final_scoring_data.shape}**************")
        print(f"Final_scoring_data_head :\n {Final_scoring_data.head(10)}")
        Final_model_vars.pop() # removing added BRE_RUN_ID here
        pred_data = Final_scoring_data[Final_model_vars]
        print(f"pred_data_head :\n {pred_data.head(10)}")
...
run()
elapsed_time = et - st
print('Txn module script execution time:', elapsed_time, 'seconds')
